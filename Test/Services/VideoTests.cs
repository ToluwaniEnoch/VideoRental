...
namespace Test.Services
{
    public class VideoTests : BaseTest
    {
        ...
        [Fact]
        public async void ReturnVideoById_InvalidPayload_UnavailableVideo_ReturnFailure()
        {
            var collection = GetCollection().BuildServiceProvider();
            var videoService = collection.GetService<IVideoService>();
            var dbContext = collection.GetService<AppDbContext>();
            ...
            await dbContext.Videos.AddRangeAsync(video1, video2, video3, video4, video5, video6);
            await dbContext.SaveChangesAsync();
            var result = videoService.GetVideoByIdAsync(Guid.NewGuid());
            Assert.Equal(ResponseCodes.BadRequest, result.Result.Code);
        }

        [Fact]
        public async void RegisterVideo_ValidPayload_ReturnFailure()
        {
            var collection = GetCollection().BuildServiceProvider();
            var videoService = collection.GetService<IVideoService>();
            var dbContext = collection.GetService<AppDbContext>();
            ...
            var result = await videoService.RegisterVideo(video);
            await dbContext.SaveChangesAsync();
